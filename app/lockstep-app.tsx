      ...
      try {
        ...
        const signature = await buildSignAndSendTrade({
          ...
          entryGuard: {
            ...
            maximumImpactPercent: migrationExecutionSettings.maxEntryImpact,
            solUsdPrice,
          },
          freshTransactionRetries: 2,
          shouldRetryFreshTransaction: async () => {
...
function SettingsDrawer({ initialMode, migrationSettings, migrationLiveSettings, newPairsSettings, onClose, onSave }: { initialMode: StrategyMode; migrationSettings: typeof migrationDefaults; migrationLiveSettings: typeof migrationLiveDefaults; newPairsSettings: typeof defaults; onClose: () => void; onSave: (migration: typeof migrationDefaults, migrationLive: typeof migrationLiveDefaults, newPairs: typeof defaults) => void }) {
  ...
  const normalizedMigration = (value: typeof migrationDefaults) => ({ ...value, maxEntryImpact: Math.min(100, Math.max(0, value.maxEntryImpact)), boostEntryMinMarketCapUsd: Math.min(value.boostEntryMinMarketCapUsd, value.boostEntryMarketCapUsd), boostEntryMarketCapUsd: Math.max(value.boostEntryMinMarketCapUsd, value.boostEntryMarketCapUsd) });
