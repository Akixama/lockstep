...
test("Migration Paper starts from its confirmed defaults instead of legacy settings", async () => {
  ...
  assert.match(source, /boostMaximumFillMarketCapUsd:\s*3700,/);
  assert.match(source, /boostReboundEnabled:\s*false,/);
  ...
  assert.match(source, /reachesMigrationEntry/);
  assert.match(source, /const reboundEnabled = migrationExecutionSettings\.boostReboundEnabled/);
  assert.match(source, /let dipArmed = false/);
  ...
  assert.match(source, /outside executable \$\{executableRangeLabel\}/);
  ...
  assert.match(source, /label="Wait for dip rebound"/);
  assert.match(source, /label="Rebound amount"/);
...
test("Fresh per-token trades trigger immediately and USD market cap falls back to SOL", async () => {
  ...
  assert.match(source, /maximumSpotMarketCapUsd:\s*migrationExecutionSettings\.boostMaximumFillMarketCapUsd/);
  ...
  assert.match(tradingSource, /verifiedMarketCapUsd > entryGuard\.maximumSpotMarketCapUsd/);
  ...
  assert.match(source, /projectedPaperFillMarketCapUsd > migrationExecutionSettings\.boostMaximumFillMarketCapUsd/);
